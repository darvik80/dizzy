...
int main(int argc, char *argv[]) {
    tson::Tileson t;
    std::unique_ptr<tson::Map> map = t.parse(fs::path("./assets/dizzy_map.json"));
    ...
    if(map->getStatus() == tson::ParseStatus::OK)
    {
        ...
        for (auto &layer : map->getLayers())
        {
            if (layer.getType() == tson::LayerType::ObjectGroup)
            {
                ...
                tson::Object *player = layer.firstObj("Hero"); //Does not exist in demo map->..
        ...
        tson::Layer *layer = map->getLayer("Main");
        tson::Tileset *tileset = map->getTileset("dizzy_tiles");
